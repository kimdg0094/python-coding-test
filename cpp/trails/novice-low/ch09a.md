...
문자열은 "글자 상자들이 순서대로 붙어 있는 것"이라고 보면 된다. `getline(cin, s)`이 돌려주는 값이 이미 그 상자 줄이다.

```text
 typed line:  apple
 getline(cin, s);      # 정수로 바꾸는 과정이 없다 , 이미 글자다
      +---+---+---+---+---+
      | a | p | p | l | e |
      +---+---+---+---+---+
        0   1   2   3   4      # 상자 하나가 글자 하나
 cout << s;   ->  apple
```

정수를 받을 때 쓰던 `cin >> n;`은 입력의 글자들을 "수 하나"로 해석하는 추가 작업이다. 이번 레슨은 글자 그대로 다루므로 그 작업을 하지 않는다.

**왜 이렇게 되는가**

- `getline(cin, s)`은 한 줄을 통째로 읽어 `string`에 담는다. 사용자가 `2024`를 입력해도 결과는 수 2024가 아니라 글자 `2`, `0`, `2`, `4`가 붙은 문자열이다.

- 그래서 `Code2024`처럼 숫자가 섞인 입력을 `int`로 받으려 하면 읽기가 실패해 값이 0이 되거나 이후 입력이 통째로 어긋난다. 반대로 `string`으로 두면 아무 문제가 없다.

- 정리하면 변환은 **필요할 때만** 한다. 계산에 쓸 값이면 `int`로 받고, 글자로 다룰 값이면 `string`으로 둔다.

- 작은따옴표와 큰따옴표를 구분하는 것도 같은 이야기다. `'a'`는 글자 하나(`char`), `"a"`는 글자 하나짜리 문자열이다. 둘은 자료형이 달라서, `s[0] == "a"`라고 쓰면 컴파일 오류가 난다. 한 글자를 비교할 때는 `s[0] == 'a'`다.

- 출력에서도 마찬가지다. `cout << s;`는 줄을 바꾸지 않는다. 파이썬 `print`가 자동으로 붙여 주던 줄바꿈이 C++에는 없으므로, 필요할 때 `cout << "\n";`을 직접 붙인다.
...
```text
 ...
 s.length() = 11    # 4 + 1 + 4 + ... : spaces are counted too
```

글자 수를 셀 때 공백을 빼먹는 실수가 잦다. `I`(1) + 공백(1) + `love`(4) + 공백(1) + `code`(4) = 11이다.

`cin >>`와 `getline`이 한 줄을 어디까지 읽는지도 함께 보자.

```text
 input line :  I love code
   cin >> s        ->  s = "I"            # 공백에서 끊긴다
   getline(cin, s) ->  s = "I love code"  # 줄 끝까지 , 개행은 버린다
```
...
마지막 인덱스는 2이지만 개수는 3이다. 즉 `s.length()`는 "마지막 인덱스 + 1"이고, 마지막 글자는 언제나 `s[s.length() - 1]`이다.

**왜 이렇게 되는가**

- `cin >>`는 공백·탭·줄바꿈을 "값의 끝"으로 본다. 그래서 문장을 받으려면 반드시 `getline`을 써야 한다.
- `s.length()`와 `s.size()`는 완전히 같은 값을 준다. 이름만 두 가지일 뿐이니 편한 쪽을 쓰면 된다.
- 다만 그 값은 **음수가 없는 정수**라, 빈 문자열에서 `s.size() - 1`은 `-1`이 아니라 아주 큰 수가 된다. `for (int i = 0; i <= s.size() - 1; i++)`라고 쓰면 빈 입력에서 반복이 멈추지 않는다. `(int)s.size()`로 감싸거나 `i < s.size()` 형태를 쓴다.
...
"문자열의 배열"은 상자 안에 또 상자가 든 2단 구조다. 바깥 인덱스는 몇 번째 단어인지, 안쪽 인덱스는 그 단어의 몇 번째 글자인지를 고른다.

```text
 string words[3] = {"red", "green", "blue"};
      +---------+---------+---------+
      |  "red"  | "green" | "blue"  |
      +---------+---------+---------+
          0          1          2         # words[1] = "green"
 ...
 words[1]     -> "green"
      ...
      +---+---+---+---+---+
        0   1   2   3   4
 words[n - 1] -> "blue"                  # the last word
```

`words[i]`는 단어 하나(문자열), `words[i][j]`는 그 단어의 글자 하나(`char`)다. 2차원 배열에서 `board[i]`가 행, `board[i][j]`가 칸이던 것과 똑같은 구조다.

**손으로 따라가기**

`3 / red / green / blue`를 읽어 배열을 채우는 과정이다.

| 단계 | 실행 | words |
| --- | --- | --- |
| 0 | `cin >> n;` 그리고 `cin.ignore();` | 아직 비어 있음 |
| 1 | `getline(cin, words[0]);` | `words[0] = "red"` |
| 2 | `getline(cin, words[1]);` | `words[1] = "green"` |
| 3 | `getline(cin, words[2]);` | `words[2] = "blue"` |

입력 순서가 곧 인덱스 순서이고, 마지막에 넣은 단어의 인덱스는 `n - 1`이다.

**왜 이렇게 되는가**

- `cin >> n;`은 숫자만 읽고 그 뒤의 **줄바꿈은 입력에 그대로 남겨 둔다.** 그 상태에서 `getline`을 부르면 남아 있던 줄바꿈을 만나 "빈 줄 하나"를 읽고 끝난다. 그래서 첫 단어가 빈 문자열이 되는 것이다.
- `cin.ignore();`가 그 남은 줄바꿈 한 글자를 버리는 역할을 한다. 숫자를 `cin >>`로 읽고 문장을 `getline`으로 읽는 조합에서는 거의 항상 필요하다. 파이썬에는 없던 단계라 가장 자주 빠뜨린다.
- 배열 크기는 컴파일할 때 정해지는 상수여야 하므로 `string words[100];`처럼 넉넉히 잡고 앞의 `n`개만 쓴다. 크기를 실행 중에 정하려면 `vector<string> words(n);`을 쓴다.
- 선언만 하고 값을 넣지 않은 `string` 칸은 쓰레기 값이 아니라 **빈 문자열**이다. `int` 배열과 달리 `string`은 스스로 빈 상태로 시작하므로 초기화를 따로 하지 않아도 된다.
...
문자열의 각 글자에는 0부터 번호가 붙는다. 파이썬과 달리 **뒤에서 세는 음수 번호는 없다.**
...
```text
 ...
 s.size() = 6   ->  valid index is 0 .. 5
 s[6]  -> out of range , no error message      # 조용히 틀린다
 s[-1] -> out of range too , C++ has no back counting
```

뒤에서 두 번째 글자가 필요하면 `s[s.size() - 2]`처럼 앞에서 세는 번호로 직접 계산한다.

순회 방법도 두 가지인데, 인덱스가 필요한지에 따라 고른다.

```text
 for (char c : s)                   -> gives the letter only : b a n a n a
 for (int i = 0; i < (int)s.size(); i++)  -> gives 0 1 2 3 4 5 , use s[i]
```
...
- `s[i]`가 돌려주는 것은 `char` 하나다. 그래서 비교할 때는 작은따옴표를 쓴다. `s[i] == "a"`처럼 큰따옴표를 쓰면 `char`와 문자열을 비교하는 셈이라 컴파일되지 않는다.
- 값만 필요하면 `for (char c : s)`가 짧고 안전하다. 자리 번호가 조건에 들어가면(`i % 2 == 0` 등) 반드시 인덱스 반복을 써야 한다.
- `int cnt;`처럼 초기값 없이 선언하면 쓰레기 값이 들어 있다. 세는 변수는 반드시 `int cnt = 0;`으로 시작한다. 파이썬에는 없던 함정이다.
- 반복 조건에서 `i < s.size()`라고만 쓰면 `int`와 음수 없는 정수를 비교해 경고가 난다. `(int)s.size()`로 맞춰 두면 경고도 사라지고, 빈 문자열에서 `size() - 1`이 거대한 수가 되는 사고도 함께 막힌다.
...
C++에서 부분 문자열은 "어디서부터 몇 글자"로 지정한다. 자르는 지점 두 개를 쓰는 파이썬 방식과 세는 법이 다르다.

```text
 s = "apple"                     s.substr(1, 3) -> "ppl"
      +---+---+---+---+---+
      | a | p | p | l | e |
      +---+---+---+---+---+
   i    0   1   2   3   4
          ^
          pos = 1                len = 3  ->  take s[1] s[2] s[3]
```

두 번째 인자는 "끝 위치"가 아니라 **길이**다. 이 한 가지만 헷갈리지 않으면 나머지는 쉽다.

```text
 s.substr(0, 2)  = "ap"     # 앞에서 2 글자
 s.substr(2)     = "ple"    # 길이를 빼면 끝까지
 s.substr(0, s.size() - 1) = "appl"    # 마지막 한 글자를 뺀 나머지
 s.substr(5)     = ""       # pos == size 는 빈 문자열 , 오류 아님
 s.substr(6)     -> throws out_of_range        # pos > size 는 예외
```

한 글자씩 이어 붙여 새 문자열을 만드는 과정은 이렇게 자란다.

```text
 result = result + s[i]      each step makes a NEW string and copies it
...
- `substr(pos, len)`의 둘째 인자가 길이라서, 잘라 낸 결과의 길이가 곧 `len`이다. 파이썬 `s[a:b]`가 `b - a`글자였던 것과 같은 개수를 얻으려면 `s.substr(a, b - a)`로 쓴다.
- `pos`가 `size()`와 같으면 빈 문자열을 돌려주지만, `size()`보다 크면 **예외를 던지며 프로그램이 멈춘다.** 파이썬이 범위를 넘는 슬라이싱을 조용히 빈 문자열로 처리하던 것과 다르다. 이 차이는 뒤의 문자열 밀기에서 다시 문제가 된다.
- **C++ `string`은 파이썬과 반대로 바꿀 수 있다(가변).** `s[0] = 'x';`가 그대로 동작한다. 그래서 "새 문자열을 만든다"는 방식은 강제가 아니라 선택이지만, 조건에 따라 다른 글자를 넣는 문제에서는 이 방식이 더 읽기 쉽다.
- `result = result + s[i]`는 오른쪽에서 새 문자열을 만들어 `result`에 다시 대입하는 것이라, 매번 지금까지의 결과를 통째로 복사한다. 위 그림처럼 전체 비용이 길이의 제곱에 비례한다.
- C++에는 더 나은 방법이 있다. `result += s[i];`는 새 문자열을 만들지 않고 **뒤에 한 글자를 덧붙인다.** 길이가 수십만이 되어도 충분히 빠르므로, 습관적으로 `+=`를 쓰는 편이 좋다.
- 문자열 리터럴끼리는 `+`로 이을 수 없다. `"ab" + "cd"`는 컴파일되지 않는다. 최소 한쪽이 `string`이어야 하므로 `string("ab") + "cd"`처럼 쓰거나, `string result = "";`에서 시작해 거기에 붙여 나간다.
...
- 못 찾았을 때의 표시로 `-1`을 쓰는 이유는 그 값이 **정답으로는 나올 수 없기** 때문이다. 인덱스는 0 이상이므로 `-1`과 겹치지 않는다.
- `bool found = false;`로 시작해 찾으면 `true`로 바꾸는 방식도 같은 뼈대다. 있는지만 알면 될 때는 이쪽이 뜻이 분명하다.
- C++에는 `s.find(target)`이라는 도구도 있다. 다만 못 찾았을 때 돌려주는 값이 `-1`이 아니라 `string::npos`라는 아주 큰 값이라, `if (s.find(c) == -1)`처럼 비교하면 틀린다. 직접 순회해 `-1`로 표시하는 방식이 이 단계에서는 덜 헷갈린다.
- `bool`을 `cout`에 그대로 내보내면 `true`/`false`가 아니라 `1`/`0`이 나온다. 문제에서 특정 단어를 요구하면 `if`로 갈라 직접 출력한다.
...
C++ `string`은 두 가지 길이 모두 열려 있다. 제자리에서 고쳐도 되고, 새 문자열을 만들어도 된다.
...
```text
 ...
 way A : change in place        s[0] = 'h';    ->  s becomes "hat"
         # C++ string is MUTABLE , this really works

 way B : build a NEW string , one letter at a time
...
- **여기가 파이썬과 정반대인 지점이다.** 파이썬 문자열은 불변이라 `s[0] = "x"`가 곧바로 오류였다. C++ `string`은 가변이라 `s[0] = 'x';`가 그대로 동작하고, 원본이 실제로 바뀐다.
- 그래서 C++에서는 방식을 고를 수 있다. 원본이 더 필요 없으면 제자리 수정(A)이 짧고 빠르다. 원본을 그대로 남겨 둬야 하거나 결과 길이가 달라지면 새 문자열(B)이 낫다.
- 다만 가변이라는 성질에는 대가가 따른다. 원본을 고쳐 버리면 되돌릴 수 없다. 뒤에서 원본이 다시 필요한 문제(예: 대소문자 무시 비교 후 원본 출력)에서는 `string t = s;`로 사본을 먼저 만들어 두고 사본만 고친다. C++의 `=`는 값을 통째로 복사하므로 이 사본은 원본과 완전히 독립이다.
- 대입할 값의 자료형에 주의한다. `s[i]`는 `char`이므로 `s[i] = 'x';`(작은따옴표)여야 하고, `s[i] = "x";`는 컴파일 오류다.
- 조건을 "값"으로 걸지 "자리"로 걸지에 따라 필요한 반복이 다르다. 값만 보면 되면 `for (char c : s)`로 충분하지만, 자리 번호가 조건에 들어가면(`i == p`, `i % 2 == 0`) 반드시 인덱스 반복이 필요하다.
- 새 문자열을 쌓을 때는 `result = result + s[i];`보다 `result += s[i];`가 낫다. 앞은 매번 통째로 복사하지만 뒤는 한 글자만 덧붙인다.
