...
```text
 s = "apple",  i = 1
   ...
   i    0   1   2   3   4
            ^                      # 이 칸만 버린다
   s.substr(0, 1) = "a"            # 앞에서 i 글자
   s.substr(2)    = "ple"          # i+1 부터 끝까지
   "a" + "ple" = "aple"
```
...
```text
 i = 4 (the last letter of "apple")
   s.substr(0, 4) = "appl"    s.substr(5) = ""     # pos == size , 빈 문자열
   "appl" + "" = "appl"
 i = 0 (the first letter)
   s.substr(0, 0) = ""        s.substr(1) = "pple"
```
...
| 식 | 뜻 | 결과 | 원본 `s` |
| --- | --- | --- | --- |
| `s.substr(0, 3) + s.substr(4)` | 인덱스 3의 글자 하나 삭제 | `banna` | `banana` |
| `for` 순회로 `'a'`가 아닌 글자만 담기 | `a`를 전부 삭제 | `bnn` | `banana` |
| `bool` 플래그로 첫 `'a'`만 건너뛰기 | 맨 앞 `a` 하나만 삭제 | `bnana` | `banana` |

세 줄 모두 **새 문자열을 만들 뿐**이므로, 결과를 쓰려면 출력하거나 변수에 담아야 한다. 계산해 놓고 아무 데도 담지 않으면 아무 일도 일어나지 않는다.

**왜 이렇게 되는가**

- `substr(pos, len)`의 둘째 인자는 끝 위치가 아니라 **길이**다. 인덱스 `i`의 글자를 지우려면 앞 조각의 길이가 `i`이므로 `s.substr(0, i)`이고, 뒤 조각은 `i+1`부터 끝까지이므로 `s.substr(i + 1)`이다.
- `pos`가 `size()`와 정확히 같으면 빈 문자열을 돌려주므로, 마지막 글자를 지울 때도 예외가 나지 않는다. 하지만 `size()`를 **넘으면** 예외가 발생하며 프로그램이 그대로 멈춘다.
- 그래서 `i`가 유효한 인덱스인지(0 이상, `size()` 미만) 먼저 확인하는 습관이 필요하다. 사람이 말하는 "K번째"를 그대로 쓰면 인덱스가 하나 밀리므로 `K - 1`로 바꾼다.
- "전부 지우기"처럼 결과 길이가 입력에 따라 달라지는 작업은 조각 자르기로 표현하기 어렵다. 그럴 때는 빈 문자열에서 시작해 남길 글자만 `+=`로 붙이는 순회가 간단하다.
- C++ `string`에는 `s.erase(i, 1);`처럼 제자리에서 지우는 기능도 있다. 원본을 바꿔도 되는 상황이면 이쪽이 짧다. 다만 원본이 뒤에 다시 필요하면 사본을 먼저 만들어 둔다.
...
```text
 s = "abcde"  (n = 5)
   left 1  : s.substr(1) + s.substr(0, 1)      "bcde" + "a"  -> "bcdea"
   right 1 : s.substr(n-1) + s.substr(0, n-1)  "e" + "abcd"  -> "eabcd"
   left k  : s.substr(k) + s.substr(0, k)          cut point = k
   right k : s.substr(n-k) + s.substr(0, n-k)      cut point = n - k
```
...
- **`k % n`을 빠뜨렸을 때 C++이 보이는 반응은 파이썬과 정반대다.** 파이썬은 범위를 넘는 슬라이싱을 빈 문자열로 처리해 "밀지 않은 원본"이 조용히 나왔다. C++의 `substr(12)`는 `12 > size()`이므로 **예외를 던지며 프로그램이 즉시 멈춘다.** 조용히 틀리는 대신 요란하게 죽는 셈이라, 원인을 찾기는 오히려 쉽다.

- 오른쪽 `k`칸이 왼쪽 `n - k`칸과 같은 이유도 주기성이다. 오른쪽으로 `k`칸 가는 것은 왼쪽으로 `n - k`칸 가는 것과 같은 자리에서 만난다.

- `k % n`을 한 뒤에도 `k`가 0이면 `substr(0)`과 `substr(0, 0)`이 되어 `s + ""`, 곧 원본이 그대로 나온다. "0칸 밀기"가 예외 처리 없이 저절로 맞는 것이다.

- C++의 `%`는 왼쪽이 음수면 결과도 음수다. `k`가 음수로 주어질 수 있는 문제라면 `k = ((k % n) + n) % n;`처럼 한 번 더 보정해 0 이상으로 만든다.
...
```text
 ...
 '7' - '0' =  7     # how far from the block start
 'c' - 'a' =  2
 'a' - 'A' = 32     # lower = upper + 32, for every letter
```

구간 안에서 번호가 이어져 있으므로 **뺄셈이 곧 "몇 번째인가"**가 되고, **덧셈이 곧 "몇 칸 뒤 글자"**가 된다. `(char)('a' + 3)`이 `'d'`인 이유다.
...
- C++에서 `char`는 사실상 작은 정수다. 그래서 `ord`·`chr` 같은 변환 함수가 아예 필요 없고, 캐스팅 `(int)`와 `(char)`가 그 역할을 한다.
- 편한 만큼 함정도 생긴다. `cout << 'A' + 1;`은 `'B'`가 아니라 `66`을 찍는다. `+`가 끼는 순간 계산은 정수로 이뤄지고, `cout`은 그 정수를 그대로 내보내기 때문이다. 글자로 보고 싶으면 반드시 `(char)('A' + 1)`처럼 감싼다.
- 반대로 `cout << c;`처럼 `char` 변수를 그냥 내보내면 글자가 나온다. 코드값을 보고 싶으면 `(int)c`로 감싼다. **같은 값이 자료형에 따라 다르게 보이는 것**이라 이해하면 헷갈리지 않는다.
- `char`끼리 빼면 결과는 정수다. `'c' - 'a'`가 `2`인 이유이고, 이 값이 곧 "그 구간에서 몇 번째"다.
- 누적 변수는 `int total = 0;`으로 시작한다. 초기값을 빼먹으면 쓰레기 값에서 더하기 시작해 답이 매번 달라진다.
- 글자가 아주 많으면 코드 합이 커질 수 있다. `int`는 약 21억까지이므로 보통은 충분하지만, 길이가 수천만이 되는 문제라면 `long long`으로 받는다.
...
`s = "12ab"`를 소문자만 대문자로 바꾸는 과정이다. C++ `string`은 가변이므로 제자리에서 고칠 수 있다.

| i | `s[i]` | `'a' <= s[i] && s[i] <= 'z'` | 실행 | s (그 직후) |
| --- | --- | --- | --- | --- |
| 0 | `1` | 거짓 | 그대로 | `12ab` |
| 1 | `2` | 거짓 | 그대로 | `12ab` |
| 2 | `a` | 참 | `s[2] = s[2] - 32;` | `12Ab` |
| 3 | `b` | 참 | `s[3] = s[3] - 32;` | `12AB` |

**왜 이렇게 되는가**

- `'a' <= c && c <= 'z'`가 소문자 판별이 되는 이유는, 문자 비교가 결국 **코드값 비교**이기 때문이다. `c`가 `'a'`(97) 이상이고 `'z'`(122) 이하라는 말은 코드가 97~122 구간에 있다는 뜻이고, 그 구간에는 소문자만 있다.

- 그래서 `'0' <= c && c <= '9'`는 숫자 글자 판별, `'A' <= c && c <= 'Z'`는 대문자 판별이 된다. 구간이 이어져 있다는 성질 하나로 세 가지 판별식이 전부 나온다.

- 파이썬처럼 `'a' <= c <= 'z'`라고 이어 쓰면 안 된다. C++은 이것을 `('a' <= c) <= 'z'`로 해석해, 앞의 결과 1 또는 0을 `'z'`(122)와 비교한다. 두 값 모두 122보다 작으므로 **언제나 참**이 되어 판별이 통째로 무력해진다. 반드시 `&&`로 나눠 쓴다.

- 판별 없이 전부 `-32`를 하면 공백(32)이 코드 0인 제어 문자가 되고, 대문자는 구간 밖으로 밀려난다. 조건이 반드시 필요한 이유다.

- `s[i] = s[i] - 32;`는 원본을 직접 고친다. 파이썬 문자열에서는 불가능했던 일이고, 여기서는 정상 동작한다. 대신 원본이 뒤에 다시 필요하면 `string t = s;`로 사본을 먼저 떠 둔다.

- `<cctype>`의 `toupper`·`tolower`를 써도 된다. 알파벳이 아닌 글자는 그대로 돌려주므로 조건 검사를 대신해 준다. 다만 돌려주는 값이 `int`라 `s[i] = (char)toupper(s[i]);`처럼 다시 `char`로 바꿔 담아야 한다.
...
```text
 string("12") + "3"     ->  "123"     # text + text  : glue the boxes
 stoi("12") + 3         ->  15        # int + int    : add the values
...
문자열 `"12"`는 상자 두 개, 정수 `12`는 값 하나다. `stoi`는 상자 줄을 읽어 값 하나로 바꾸는 변환이고, 그 반대가 다음 레슨의 `to_string`이다.

```text
 stoi(" 42 ")  ->  42     # 앞쪽 공백은 건너뛴다
 stoi("-7")    ->  -7     # 맨 앞 음수 부호는 인정된다
 stoi("007")   ->   7     # 앞의 0 은 사라진다
 stoi("12a")   ->  12     # 읽을 수 있는 데까지만 읽는다 ( 예외 아님 )
 stoi("3.5")   ->   3     # 점에서 멈춘다
 stoi("abc")   ->  throws invalid_argument      # 시작부터 숫자가 아니면 예외
 stoi("")      ->  throws invalid_argument
```

**손으로 따라가기**

`s = "123"`의 자릿수 합을 구하는 과정이다. 전체를 한 번에 `stoi(s)`로 바꾸는 것과는 다른 작업이다.
...
한 글자짜리 숫자 문자는 `c - '0'`으로 바로 값이 된다. `'0'`이 48이고 그 뒤로 코드가 이어지므로, 뺄셈 결과가 곧 그 자리의 숫자다.

**왜 이렇게 되는가**

- `+`가 무엇을 하는지는 양쪽 자료형이 정한다. `string`끼리면 이어붙이기, `int`끼리면 덧셈이다. 그래서 계산하려면 먼저 정수로 바꿔야 한다.
- 입력을 `cin >> a;`로 `int`에 바로 받으면 변환이 저절로 이뤄진다. 굳이 문자열로 받았다가 `stoi`로 바꿀 필요가 없는 경우가 많다.
- **파이썬 `int()`와 다른 점이 하나 있다.** 파이썬은 `int("12a")`를 오류로 막지만, C++의 `stoi("12a")`는 읽을 수 있는 앞부분만 읽어 `12`를 돌려준다. 조용히 통과하므로, 입력 형식이 확실하지 않으면 이 차이가 오답으로 이어진다.
- 값이 `int` 범위를 넘으면 `stoi`는 예외를 던진다. 20자리 숫자 문자열 같은 것을 다뤄야 하면 `stoll`로 `long long`을 쓰거나, 아예 한 글자씩 문자열로 처리한다.
- 자릿수 합처럼 각 자리를 따로 봐야 하는 문제는 문자열을 순회하며 `c - '0'`을 더하는 편이 낫다. 정수로 바꾼 뒤 10으로 나누며 세는 방법과 결과는 같지만, 자릿수가 많아도 안전하다.
- 앞의 0을 무시해야 하는 문제에서는 `stoi`가 그 일을 대신해 준다. `stoi("007")`이 `7`이므로 따로 처리할 것이 없다.
...
`to_string`은 값 하나를 글자 상자 줄로 펼친다. 펼치는 순간 인덱싱·`substr`·`size()` 같은 문자열 도구를 전부 쓸 수 있다.

```text
 n = 1000  (one number)      to_string(n) = "1000"  (four boxes)
                              +---+---+---+---+
                              | 1 | 0 | 0 | 0 |
                              +---+---+---+---+
                                0   1   2   3
 to_string(n).size() = 4       # digits of the number
 to_string(n)[0]     = '1'     # the leading digit
 reversed            = "0001"  # 뒤집으면 앞자리 0 이 그대로 남는다
```

마지막 줄이 중요하다. **뒤집기는 문자열의 연산**이라 앞자리 0이 그대로 남는다. 이 결과를 다시 `stoi`로 바꾸면 0이 사라져 1이 된다.

```text
 string("score: ") + 90              # 뜻이 달라지거나 컴파일 오류
 string("score: ") + to_string(90)   -> "score: 90"
 to_string(-7)                       -> "-7"    # 부호도 상자 하나
 to_string(-7).size()                -> 2       # 그래서 길이에 포함된다
```

**왜 이렇게 되는가**

- 자릿수를 `to_string(n).size()`로 셀 수 있는 이유: 10진수 표기는 자릿수 하나가 글자 하나에 정확히 대응한다. 그래서 "글자 수 = 자릿수"가 그냥 성립한다. 10으로 계속 나누며 세는 방법과 결과가 같지만 훨씬 짧다.

- 단 음수에서는 어긋난다. `to_string(-7)`은 `'-'`까지 상자로 만들기 때문에 길이가 2다. 자릿수만 세려면 부호를 뺀 값으로 바꿔야 한다.

- 팰린드롬 판정을 문자열로 하는 이유도 같다. 수 상태에서는 "뒤집기"가 나눗셈·나머지의 반복이지만, 문자열이면 앞뒤에서 한 글자씩 맞춰 보거나 `<algorithm>`의 `reverse(t.begin(), t.end());` 한 줄이면 끝나고 비교도 `==` 하나로 된다.

- `reverse`는 원본을 **제자리에서** 뒤집는다. 원본이 그대로 필요하면 `string t = s;`로 사본을 먼저 만들고 `t`를 뒤집는다. 파이썬의 `s[::-1]`이 새 문자열을 돌려주던 것과 다른 점이다.

- 정수와 문자열은 `+`로 바로 합칠 수 없다. `"score: " + 90`은 이어붙이기가 아니라 포인터 계산으로 해석되어 엉뚱한 결과가 나오거나 컴파일 오류가 난다. 반드시 `to_string`으로 바꿔서 붙인다.

- 정수를 문자열로 바꾸는 순간 계산은 못 하게 된다. 계산이 더 필요하면 수로 두고, 자르기·세기·이어붙이기가 필요할 때만 바꾼다.
...
| 1 | `string la = a, lb = b;` | 비교용 사본을 **복사해서** 따로 만든다 |
| 2 | `la`, `lb`를 소문자로 | `la = "cat"`, `lb = "car"` |
| 3 | `la == lb` → 거짓 | 같지 않으므로 크기 비교로 넘어간다 |
| 4 | 첫 글자 `c` = `c`, 둘째 `a` = `a` | 아직 승부가 나지 않음 |
| 5 | 셋째 `t`(116) > `r`(114) | `la > lb`, 즉 `lb` 쪽이 앞선다 |
| 6 | 출력 `Car` | 앞선 쪽의 **원본**을 낸다(`car`가 아님) |
...
- `string`끼리는 `==`, `<`, `>`가 이 규칙대로 동작한다. 다만 **양쪽이 모두 `string`이어야** 한다. `char*` 두 개를 `==`로 비교하면 글자가 아니라 주소를 비교하게 되므로, 한쪽을 `string(...)`으로 감싸거나 애초에 `string` 변수로 받는다.

- C++ `string`은 가변이라, `a`를 소문자로 바꿔 버리면 원본이 실제로 사라진다. 그래서 비교용 사본을 `string la = a;`로 **복사해 두고** 사본만 고친다. `=`가 값을 통째로 복사하므로 이 사본은 원본과 완전히 독립이다.

- 파이썬에서는 `a.lower()`가 새 문자열을 돌려주기 때문에 원본이 저절로 보존됐다. C++에서는 제자리 수정이 기본이라 이 보존을 **직접** 해야 한다는 점이 다르다.

- 비교 결과인 `bool`을 `cout`에 그대로 내보내면 `1`/`0`이 나온다. 문제가 특정 단어를 요구하면 `if`로 갈라 직접 출력한다.
