...
- 이 레슨은 Ch9(그래프 탐색)의 핵심을 **반복 훈련**하고, 코딩테스트 단골 유형으로 **확장**하는 연습 세트다. 새 문법은 없다. 인접리스트(`vector<vector<int>>`)·`visited`·DFS(재귀 또는 `vector` 스택)·BFS(`queue<int>`)·격자 4방향 탐색만으로 12문제를 푼다.

- **반복 훈련 개념**
- 인접리스트 구성: 무방향은 양쪽, 방향은 한쪽만 — `adj[u].push_back(v); adj[v].push_back(u);`
- DFS(재귀/스택) + 방문 표시: 진입 즉시 마킹, 이웃은 정렬해 번호 순으로 — `for (int v : adj[u]) if (!visited[v]) dfs(v);`
- BFS + 거리 배열: 큐에 넣는 순간 거리 확정 — `int u = q.front(); q.pop(); ... dist[v] = dist[u] + 1; q.push(v);` (`pop()`은 값을 돌려주지 않으므로 `front()`로 먼저 읽는다)
- 연결 요소: 미방문 정점을 만날 때마다 탐색을 새로 시작하고 개수·크기를 센다 — `for (int s = 0; s < N; s++) if (!visited[s]) { ... }`
- 격자 탐색: 칸=정점, 4방향(또는 8방향)=간선, 경계 검사 필수 — `nx >= 0 && nx < H && ny >= 0 && ny < W`
...
- **셀프체크**: 1-based라 `vector<vector<int>> adj(N + 1)`처럼 크기를 `N+1`로 잡았는가? 무방향이므로 `u`의 목록에 `v`, `v`의 목록에 `u`를 **둘 다** `push_back` 했는가? 친구 수 `0`인 사람도 빠뜨리지 않고 한 줄을 출력했는가?
...
- "각 정점의 이웃 목록"이 곧 인접리스트다. `vector<vector<int>>` 하나면 되고, 간선을 읽으면서 양쪽 벡터에 서로를 `push_back` 하기만 하면 자료구조가 완성된다.
- 출력은 오름차순이어야 하므로 벡터마다 `sort` 한 번. 간선이 들어온 순서는 제멋대로일 수 있으니 정렬 없이 출력하면 정답이 유일하지 않다.
- 복잡도: 간선 저장 `O(M)`, 정렬은 벡터 길이 합이 `2M`이라 `O(M log M)` 이내.

(2) 코드 단계별

- 1-based 번호를 그대로 쓰기 위해 `vector<vector<int>> adj(N + 1)`로 `N+1`칸을 만든다(0번은 비워 둠).
- 간선 `u v`마다 `adj[u].push_back(v)`, `adj[v].push_back(u)` — 무방향이라 양쪽.
- `1..N` 순서로 `sort(adj[i].begin(), adj[i].end())` 후 `adj[i].size()`를 먼저 출력하고, 범위 기반 `for`로 번호 앞에 공백을 붙여 이어 쓴다.
- 출력 줄 끝은 `endl` 대신 `'\n'`. `endl`은 매번 버퍼를 비워 줄 수가 많을 때 느리다.

(3) 스스로 다시 짤 때 생각 순서

- "N+1 크기 인접리스트" → "간선마다 양쪽 push_back" → "각 벡터 정렬" → "친구 수 + 목록 출력". 경계값은 `M = 0`(전부 `0` 한 줄씩)과 한 사람에게 친구가 몰린 경우(예제 3)다.
- C++ 함정: `adj[i].size()`의 타입은 `size_t`(부호 없는 정수)다. 그대로 출력하는 건 문제없지만 `adj[i].size() - 1`처럼 뺄셈을 하면 크기가 0일 때 거대한 값이 되므로, 뺄 때는 `(int)adj[i].size()`로 바꿔 쓴다.
```
...
- **셀프체크**: 방향 간선이므로 `adj[u]`에만 넣었는가(양쪽에 넣으면 4번 역이 방문돼 오답)? 인접리스트를 정렬해야 "작은 번호 먼저"가 보장된다. 미도달 수는 `N - (int)order.size()`로 바로 구한다 — `size()`가 부호 없는 타입이라 캐스팅 없이 빼면 결과가 `unsigned`로 승격돼 엉뚱한 값이 나올 수 있다.
...
- `N ≤ 100`이라 재귀 깊이는 최대 100 — C++ 기본 스택(보통 1MB)에서 전혀 문제없다. 복잡도 `O(N + M)`(+정렬).

(2) 코드 단계별

- 간선을 `adj[u].push_back(v)`로 한쪽만 저장하고 각 벡터를 정렬(작은 번호 먼저).
- `dfs(u)`: `visited[u] = true`, `order.push_back(u)` 후 정렬된 이웃을 보며 미방문이면 재귀.
- 재귀 함수가 `adj`·`visited`·`order`를 모두 봐야 하므로 전역으로 두었다. 지역 변수로 두고 싶다면 함수에 **참조(`vector<vector<int>>& adj`)** 로 넘긴다 — 값으로 넘기면 호출마다 그래프 전체가 복사돼 느려진다.
- 예제 1 검산: 0 → 이웃 [1, 2] 중 1 → 1의 이웃 3 → 3은 이웃 없음, 복귀 → 0의 다음 이웃 2 → 2의 이웃 3은 방문됨. 순서 `0 1 3 2`, 미도달은 4 하나.
- 마지막에 `N - (int)order.size()`가 도달 못한 역 수.

(3) 스스로 다시 짤 때 생각 순서

- "방향 인접리스트(한쪽만) + 정렬" → "visited/order 준비" → "s에서 재귀 DFS" → "순서 출력, N − 방문 수 출력". 예제 3처럼 사이클이 있어도 `visited` 덕분에 멈춘다.
- C++ 함정: 재귀 깊이가 수십만이 되는 그래프(일자로 늘어선 10만 정점 등)에서는 스택 오버플로가 난다. 예외도 메시지도 없이 프로세스가 그냥 죽으므로 원인을 찾기 어렵다 — 그럴 땐 아래 4번 문제처럼 명시적 `vector` 스택으로 바꾼다.
```
...
- 무방향 간선을 양쪽에 저장하고 `vector<int> dist(N, -1)`, `dist[0] = 0`.
- BFS: `q.front()`로 읽고 `q.pop()`으로 버린 뒤, 이웃 중 `dist == -1`인 사람만 `dist[u] + 1`로 확정하며 `q.push(v)`(처음 도달 = 최단).
- `*max_element(dist.begin(), dist.end())`가 마지막 감염일. 크기 `maxd + 1`의 `cnt`를 만들어 `-1`이 아닌 값만 누적.
...
- C++ 함정: `queue`의 `pop()`은 **값을 돌려주지 않는다**(반환형 `void`). 반드시 `front()`로 먼저 읽어야 한다. 게다가 `front()`가 돌려주는 건 참조라 `pop()` 뒤에는 무효가 되므로, `int u = q.front();`처럼 **값으로 복사**해 두고 `pop()`하는 순서를 지킨다.
```
...
- **셀프체크**: 미방문 정점에서 탐색을 새로 시작할 때마다 "그룹 하나"가 생기고, 그 탐색에서 꺼낸 정점 수가 그룹 크기다. 간선 `3 1`처럼 이미 같은 그룹인 쌍이 다시 와도 크기가 중복 계산되지 않는가? 혼자인 학생도 크기 `1`인 그룹이다. 내림차순 정렬은 `sort(v.begin(), v.end(), greater<int>())`.
...
- 스택은 `vector<int>`로 충분하다: `push_back`(넣기), `back()`(꼭대기 읽기), `pop_back()`(버리기). `stack<int>`를 써도 되지만 `top()`/`pop()`으로 이름만 달라진다.
- 각 시작점 `s`에서 스택 DFS: 꺼낼 때 `size++`, 미방문 이웃은 마킹하며 `push_back`.
- 탐색이 끝나면 `sizes`에 크기를 추가. 모든 정점 처리 후 `greater<int>()`로 내림차순 정렬해 개수와 목록을 출력.
...
- C++ 함정: `pop_back()`도 `pop()`처럼 **값을 반환하지 않는다**. `int x = st.back(); st.pop_back();` 두 줄이 한 세트다. 순서를 바꾸면 이미 지워진 원소를 읽는 것이라 정의되지 않은 동작이다.
```
...
- **셀프체크**: 출력이 "지나는 칸 수"가 아니라 "이동 횟수"다(`S`와 `E`가 인접하면 `1`). `S`와 `E`의 위치를 격자에서 먼저 찾았는가? 격자 한 줄은 `string`으로 읽고 `grid[i][j]`로 칸을 본다 — `E`도 `#`이 아니므로 통과 조건 `!= '#'`으로 걸러야 큐에 들어간다.
...
- 격자는 `vector<string>`으로 읽는다. `cin >> grid[i]`는 공백까지 끊어 읽으므로 한 줄이 통째로 들어온다.
- 방향은 `int dx[4]`, `int dy[4]` 두 배열로 두고 `d = 0..3`을 돈다.
...
- C++ 함정: 경계 검사를 빼먹고 `grid[nx][ny]`를 먼저 보면 안 된다. `nx`가 `-1`이어도 컴파일도 되고 예외도 없이 엉뚱한 메모리를 읽어 **틀린 답이 조용히** 나온다. 조건식에서 범위 검사를 항상 `&&`의 맨 앞에 두면 단축 평가 덕분에 안전하다.
```
...
- 무방향 간선을 양쪽에 저장.
- `bool dfs(int u, int parent)`: `u`를 마킹하고 이웃 `v`를 본다. 미방문이면 `dfs(v, u)`를 재귀 호출해 그 안에서 사이클이 발견되면 즉시 `true`를 위로 전파. 이미 방문했는데 `v != parent`이면 사이클 → `true`.
- 최초 호출은 `dfs(s, -1)` — 정점 번호가 0부터라 "부모 없음"을 `-1`로 쓴다.
...
- C++ 함정: 재귀 함수가 `bool`을 돌려주는데 어느 경로에서 `return`을 빠뜨리면 반환값이 쓰레기가 된다(컴파일은 경고만 내고 통과한다). 마지막 `return false;`를 반드시 남긴다.
```
...
- `vector<int> color(N, -1)`로 "미칠함"을 표시. 정점 `s`를 순서대로 보며 미칠함이면 `color[s] = 0`으로 두고 BFS 시작.
- 이웃 `v`가 미칠함이면 `1 - color[u]`로 칠하고 큐에 넣는다. 이미 칠해져 있고 `color[v] == color[u]`면 `ok = false`로 끝낸다.
- 바깥 `for`의 조건에 `&& ok`를 넣어 충돌이 나면 남은 덩어리를 보지 않고 빠져나온다.
...
- C++ 함정: `color`를 `vector<bool>`로 두면 "아직 안 칠함"을 표현할 자리가 없다. 상태가 셋(미칠함/A/B)이면 `int`로 두고 `-1`을 미칠함으로 쓰는 편이 안전하다.
```
...
- **셀프체크**: 8방향은 `dx, dy ∈ {-1, 0, 1}`의 조합 9개 중 `(0, 0)`을 뺀 8개다. 대각선으로만 붙은 땅(예제 1의 (1,4)와 (2,3))이 같은 섬으로 묶이는가? 격자를 `vector<string>`으로 읽었다면 비교 대상은 정수 `1`이 아니라 **문자 `'1'`** 이다. 크기는 "스택에서 꺼낸 횟수"로 세면 중복이 없다.
...
- 격자를 `vector<string>`으로 읽고 `vector<vector<bool>> visited`를 준비.
- 모든 칸을 훑다 "땅 + 미방문"이면 `count++`, `vector<pair<int,int>>` 스택으로 DFS 시작. 스택에 넣을 때 마킹.
- 꺼낼 때 `size++`, 8방향 이웃 중 경계 안·미방문·땅만 `push_back`.
- 섬 하나가 끝나면 `largest`를 갱신. 마지막에 `count largest` 출력.
...
- C++ 함정: `grid[i][j]`는 `char`다. `if (grid[i][j] == 1)`이라고 쓰면 숫자 1(제어문자)과 비교하는 것이라 항상 거짓이 되어 섬이 0개로 나온다. 반드시 작은따옴표로 `'1'`을 쓴다. 숫자로 쓰고 싶다면 `grid[i][j] - '0'`으로 변환한다.
```
...
- **셀프체크**: BFS에서 정점을 **처음 발견한 순간** `parent[v] = u`를 기록하고, `t`에서 `parent`를 거슬러 올라가 `reverse`하면 경로다. 인접리스트를 오름차순으로 정렬해 두면 처음 발견되는 부모가 사전순으로 가장 앞선 경로를 만든다. `s == t`일 때 경로가 `s` 하나로 나오는가?
...
- 무방향 간선을 양쪽 저장 후 각 벡터 정렬.
- `vector<int> parent(N, -1)`(시작점의 부모는 `-1`로 남아 역추적 종료 조건이 된다). `s`를 마킹하고 BFS.
- 이웃을 처음 발견하면 마킹 + `parent[v] = u` + `q.push(v)`. 이미 방문한 정점의 부모는 덮어쓰지 않는다(덮어쓰면 최단성이 깨진다).
- `visited[t]`가 거짓이면 `-1`. 아니면 `for (int cur = t; cur != -1; cur = parent[cur])`로 모은 뒤 `reverse(path.begin(), path.end())`.
...
- C++ 함정: `reverse`는 `<algorithm>`의 함수이고 컨테이너의 멤버가 아니다. `path.reverse()`는 컴파일 오류이므로 `reverse(path.begin(), path.end())`로 쓴다.
```
...
- **셀프체크**: 불은 시작점이 여러 개인 **다중 시작 BFS**로 각 칸의 도착 시각 `fire`를 먼저 구한다(닿지 않는 칸은 `INF = 1e9`). 사람 BFS에서 `d+1` 시각에 들어갈 칸은 `fire[nx][ny] > d+1`이어야 한다(같으면 불과 동시에 도착 → 불가). 격자 밖으로 나가는 이동을 "탈출 성공, 시각 `d+1`"로 처리했는가? 사람이 이미 가장자리에 있으면 답은 `1`이다.
...
int main() {
    ...
    for (int i = 0; i < H; i++) {
        for (int j = 0; j < W; j++) {
            if (grid[i][j] == 'F') { fire[i][j] = 0; q.push(make_pair(i, j)); }
...
- C++ 함정: "무한대"를 `INT_MAX`로 두면 `fire[x][y] + 1`이 오버플로해 음수가 되어 비교가 뒤집힌다. 이 문제처럼 값에 1씩 더할 거라면 `INF = 1e9`(int)처럼 **더해도 넘치지 않을 여유가 있는 큰 수**를 쓴다.
```
...
int main() {
    ...
    vector<string> grid(H);
    for (int i = 0; i < H; i++) cin >> grid[i];
    vector<vector<array<int, 2>>> dist(H, vector<array<int, 2>>(W, array<int, 2>{-1, -1}));
    dist[0][0][0] = 0;
    queue<array<int, 3>> q;
        ...
        for (int d = 0; d < 4; d++) {
            ...
            if (grid[nx][ny] == '0') {
                if (dist[nx][ny][b] == -1) {
                    ...
                    q.push(array<int, 3>{nx, ny, b});
                }
            } else if (b == 0) {
                if (dist[nx][ny][1] == -1) {
                    dist[nx][ny][1] = dist[x][y][0] + 1;
                    q.push(array<int, 3>{nx, ny, 1});
                }
            }
        }
    }
    int d0 = dist[H - 1][W - 1][0];
    int d1 = dist[H - 1][W - 1][1];
    int ans = -1;
    if (d0 != -1) ans = d0;
    if (d1 != -1 && (ans == -1 || d1 < ans)) ans = d1;
    cout << ans << '\n';
    return 0;
}
@@TESTS
...
- `dist`는 `vector<vector<array<int,2>>>`로 3차원을 만든다(`[x][y][b]`). 전부 `-1`로 초기화하고 `dist[0][0][0] = 0`, 큐에 `{0, 0, 0}`.
- 큐 원소는 좌표 둘 + 상태 하나라 `array<int,3>`이 깔끔하다(`pair`로는 셋을 담을 수 없고 `tuple`은 꺼낼 때 `get<0>`이 번거롭다).
...
- C++ 함정: 마지막 최솟값을 `min(d0, d1)`로 쓰면 안 된다. `-1`은 "도달 못 함"인데 수치로는 가장 작아서 도달 가능한 답을 밀어내고 `-1`이 뽑힌다. `-1`을 걸러낸 뒤 비교해야 한다.
```
...
- **셀프체크**: `k`마다 `visited`를 **새로** 만들어(또는 `assign`으로 전부 `false`로 되돌려) 연결 요소를 다시 세야 한다(재사용하면 두 번째 `k`부터 0이 나온다). "높이 `> k`인 칸만 정점"이라는 조건을 탐색 시작과 이웃 확장 양쪽에 모두 걸었는가? 동률일 때 가장 작은 `k`를 남기려면 `>`로만 갱신해야 한다(`>=`면 큰 `k`로 덮인다). 모두 잠기면 안전 지대는 `0`개다.
...
- 격자를 `vector<vector<int>>`로 읽으며 `maxh`를 같이 구한다. `best = -1`로 시작해 `k = 0`의 결과가 반드시 기록되게 한다.
- 각 `k`마다 `visited`를 새로 만들고(루프 안에서 선언하면 매번 새 배열이다), "높이 `> k` + 미방문"인 칸마다 `cnt++` 후 스택 DFS로 덩어리를 마킹(이웃도 높이 `> k`만).
- `cnt > best`일 때만 `best`, `bestK` 갱신. 마지막에 `best bestK` 출력.
...
- C++ 함정: `visited`를 루프 **밖**에서 한 번만 만들면 이전 `k`의 흔적이 남아 두 번째 `k`부터 개수가 0이 된다. 성능이 걱정되면 매번 새로 만드는 대신 `for (auto& row : visited) row.assign(W, false);`로 되돌린다 — 어느 쪽이든 "매 `k`마다 초기화"가 핵심이다.
```
