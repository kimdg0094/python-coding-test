...
- 이 레슨은 Ch10(그래프 알고리즘)의 핵심 다섯 가지 — Dijkstra, Floyd-Warshall, Kruskal(Union-Find), Prim, 위상 정렬(Kahn) — 를 **반복 훈련**하고, 코딩테스트 단골 유형으로 **확장**하는 연습 세트다. 새 문법은 없다. `priority_queue`(최소 힙으로 쓰려면 `greater<>`), 2차원 `vector`, `queue`, 배열 기반 Union-Find만으로 12문제를 푼다.

- **반복 훈련 개념**
- Dijkstra: 최소 힙에서 `(거리, 정점)`을 꺼내 낡은 항목은 스킵, 이웃을 완화 — `if (d > dist[u]) continue;` … `if (d + w < dist[v]) { dist[v] = d + w; pq.push({dist[v], v}); }` (힙 선언은 `priority_queue<pair<long long,int>, vector<pair<long long,int>>, greater<>> pq;`)
...
- Prim: 힙에 "누적 거리"가 아니라 **간선 가중치 그 자체**를 넣는다 — `pq.push({wv, v});`
- 위상 정렬(Kahn): 진입차수 0을 큐에 넣고 꺼내며 이웃 차수 감소, 처리 수가 `< N`이면 사이클 — `if (--indeg[v] == 0) q.push(v);`
...
- **셀프체크**: 무방향이므로 간선을 `graph[u]`, `graph[v]` **양쪽**에 넣었는가? `s == t`이면 답은 `0`이다. 힙에서 꺼낸 거리가 `dist[u]`보다 크면 낡은 항목이라 스킵(지연 삭제)했는가? `priority_queue`는 기본이 **최대 힙**이므로 세 번째 템플릿 인자로 `greater<>`를 직접 지정해야 최소 힙이 된다 — 빠뜨리면 가장 먼 것부터 꺼내 최단 거리가 아니라 엉뚱한 값이 나온다. `INF`를 `-1`로 바꾸는 건 출력 직전에만.
...
- 가중 그래프의 인접리스트는 `vector<vector<pair<int, long long>>>` — `(이웃 번호, 가중치)` 쌍을 담는다. 무방향이므로 `graph[u]`와 `graph[v]` 양쪽에 넣는다.
- 힙은 `priority_queue<pair<long long,int>, vector<pair<long long,int>>, greater<pair<long long,int>>>`. `pair`의 **첫 원소가 거리**여야 거리 기준으로 정렬된다.
...
- C++ 함정 두 가지. (a) `greater<>`를 빼면 `priority_queue`는 최대 힙이라 가장 **먼** 것부터 꺼내게 되어 사실상 최장 경로 비슷한 순회가 된다 — 컴파일도 되고 답도 나오므로 알아채기 어렵다. (b) 거리 누적은 `long long`으로. 간선이 1000개, 가중치가 1000이면 이 문제는 `int`로도 되지만, 가중치가 커지는 순간 `int`는 21억에서 조용히 넘친다.
```
...
- **셀프체크**: 질의가 많고 `N`이 작으니 Floyd-Warshall로 전 쌍을 한 번에 구하고 질의는 `O(1)` 조회가 낫다. 같은 도시 쌍에 도로가 여러 개면 초기화 때 짧은 것을 남겼는가? 삼중 루프에서 `k`가 **가장 바깥**인가? `a == b`는 `0`.
...
- `vector<vector<long long>> dist(n+1, vector<long long>(n+1, INF))`로 표를 만들고 대각선만 `0`. 무방향 도로는 `dist[u][v]`와 `dist[v][u]` 양쪽에, 중복 도로는 더 짧을 때만 덮어쓴다.
- 삼중 루프 `k → i → j`. `dist[i][k]`나 `dist[k][j]`가 `INF`면 건너뛴다 — 불필요한 덧셈을 줄이는 최적화이자, 아래에서 설명할 오버플로 방지책이다.
- 질의 `a b`마다 `dist[a][b]`가 `INF`면 `-1`, 아니면 그 값을 출력.
- 예제 1 검산: 1→3은 직통 10보다 1→2→3 = 7이 짧고, 1→4 = 7 + 1 = 8.

(3) 스스로 다시 짤 때 생각 순서

- "INF 행렬 + 대각선 0" → "간선 입력(양방향, 짧은 것만)" → "k 최외곽 삼중 루프" → "질의 조회". 경계값: `a == b`(0), 다른 덩어리(예제 2), 같은 쌍의 도로 2개(예제 3). 계산 중에는 `INF`를 유지하고 출력 때만 `-1`로 바꿔야 잘못된 경유가 생기지 않는다.
- C++ 함정: `INF = 1e18`을 두 개 더하면 `2e18`로 `long long` 한계(약 `9.2e18`)에는 안 걸리지만, `INF`를 더 키우거나 `int`로 두면 즉시 넘쳐 음수가 되고 "INF보다 짧은 경로"로 오인된다. 안전한 습관은 둘 중 하나다 — 더하기 **전에** `INF`인지 검사하거나(이 코드), `INF`를 `1e9`처럼 "더해도 안전한 큰 수"로 잡거나.
```
...
- **셀프체크**: 정렬 전에 `u > v`면 `swap(u, v)`로 바꿔서 `{w, u, v}` 순서로 저장해야 동률 간선의 검사 순서가 문제 규칙과 같아진다(`array<int,3>`은 앞 원소부터 사전식으로 비교되므로 담는 순서가 곧 정렬 기준이다). `find`에 경로 압축을 넣었는가? `N = 1`이면 채택 간선 0개로 `0`만 출력된다.
...
int main() {
    ...
    for (size_t i = 0; i < edges.size(); i++) {
        ...
        if (ru != rv) {
            p[ru] = rv;
            total += w;
            chosen.push_back(array<int, 3>{u, v, w});
    ...
    if ((int)chosen.size() != n - 1) {
        ...
        for (size_t i = 0; i < chosen.size(); i++) {
            cout << chosen[i][0] << ' ' << chosen[i][1] << ' ' << chosen[i][2] << '\n';
        }
    }
    return 0;
}
...
- 비용이 같은 간선이 여러 개면 어떤 걸 먼저 보느냐에 따라 채택 목록이 달라진다. 그래서 문제는 `(비용, 작은 번호, 큰 번호)` 순서를 못 박았고, `array<int,3>`을 그 순서로 만들어 `sort`하면 배열의 사전식 비교가 규칙을 그대로 구현한다.
- 복잡도: 정렬 `O(M log M)`이 지배적.

(2) 코드 단계별

- 간선 입력 시 `u > v`면 `swap(u, v)`로 항상 `u < v`로 두고 `{w, u, v}`를 모아 정렬.
- `p`를 `p[i] = i`로 초기화해 각자 자기 대표. `find`는 `p[x] = p[p[x]]`로 한 단계씩 끌어올리는 경로 압축(재귀 없이 `while`로 쓰면 스택 걱정이 없다).
...
- C++ 함정: `chosen.size()`는 부호 없는 타입이라 `chosen.size() != n - 1`에서 `n = 1`이면 `n - 1`이 `0`으로 올라가 우연히 맞지만, `n`이 `int`이고 표현식에 뺄셈이 섞이면 부호 혼합 비교 경고가 난다. `(int)chosen.size()`로 명시적으로 캐스팅하는 습관을 들인다.
```
...
int main() {
    ...
    vector<int> path;
    for (int cur = t; cur != 0; cur = parent[cur]) path.push_back(cur);
...
- `vector<int> parent(n + 1, 0)`으로 초기화(정점 번호는 1부터라 `0`은 "없음"). `dist[s] = 0`, 힙 시작.
- 완화 성공(`nd < dist[v]`)할 때만 `parent[v] = u`.
- `dist[t]`가 `INF`면 `-1`. 아니면 `for (int cur = t; cur != 0; cur = parent[cur])`로 모아 `reverse`.
...
- C++ 함정: 경로 복원 루프의 종료 조건이 `cur != 0`인데, 만약 정점 번호를 0부터 쓰는 문제라면 `0`이 진짜 정점이라 무한 루프가 된다. 0-based일 때는 `parent`를 `-1`로 초기화하고 `cur != -1`로 바꾼다.
```
...
- **셀프체크**: 경로는 `s→a→b→t` 또는 `s→b→a→t` 둘 중 하나다. `s`, `a`, `b`에서 각각 Dijkstra를 돌리면(총 3번) 필요한 거리 6개가 모두 나온다(무방향이라 `dist_a[b] == dist_b[a]`). 어느 한 구간이라도 `INF`면 그 순서는 불가능 — C++에서는 `INF + 유한값`이 `INF`로 남지 않으므로, 세 구간을 더하기 **전에** 각각 `INF`인지 검사해야 한다. `a == b`나 `s == a` 같은 겹침도 그대로 동작하는가?
...
int main() {
    ...
    int s, a, b, t;
    cin >> s >> a >> b >> t;
    vector<long long> ds = dijkstra(n, graph, s);
    vector<long long> da = dijkstra(n, graph, a);
    vector<long long> db = dijkstra(n, graph, b);
    long long r1 = INF;
    if (ds[a] != INF && da[b] != INF && db[t] != INF) r1 = ds[a] + da[b] + db[t];
    long long r2 = INF;
    if (ds[b] != INF && db[a] != INF && da[t] != INF) r2 = ds[b] + db[a] + da[t];
    long long ans = min(r1, r2);
    if (ans == INF) cout << -1 << '\n';
    else cout << ans << '\n';
    return 0;
}
...
- 어느 구간이 끊기면 그 순서는 불가능하므로 합을 만들지 않고 `INF`로 남긴다. 복잡도 `O(3 · M log N)`.

(2) 코드 단계별

- Dijkstra를 함수로 분리해 `ds`, `da`, `db` 세 배열을 만든다. 그래프는 `const vector<...>&`로 **참조** 전달 — 값으로 넘기면 호출마다 인접리스트 전체가 복사된다.
- `r1 = ds[a] + da[b] + db[t]`(a 먼저), `r2 = ds[b] + db[a] + da[t]`(b 먼저). 각각 세 구간이 모두 유한할 때만 더하고, 아니면 `INF`로 둔다. 둘의 최솟값이 `INF`면 `-1`.
...
- "경유 조건 → 구간 분할" → "필요한 시작점마다 Dijkstra" → "구간이 모두 유한할 때만 합산" → "둘 다 불가면 -1". 경계값: 창고가 다른 덩어리(예제 2), 창고가 출발점과 같은 경우(예제 3), 두 창고가 같은 정점이면 `da[b] = 0`이라 자연스럽게 처리된다.
- C++ 함정: `INF`를 그냥 더해 버리면 `1e18 + 1e18 + 1e18 = 3e18`이 되어 "도달 불가"인데도 `INF`와 값이 달라 `-1` 판정이 무너진다(`INF`를 조금만 더 키우면 `long long`을 넘겨 음수가 된다). 무한대끼리 더하지 않는 것이 원칙이다.
```
...
- **셀프체크**: 모든 친구 집에서 모든 동네까지 거리가 필요하니 Floyd-Warshall이 편하다. 어떤 친구라도 `x`에 못 가면 그 `x`는 후보에서 빠진다 — 합에 `INF`를 섞는 대신 "한 명이라도 `INF`면 그 `x`는 건너뛴다"로 명시적으로 거른다. 동률에서 작은 번호를 남기려면 `x`를 오름차순으로 돌며 **엄격히 작을 때만** 갱신한다. 모임 장소가 친구 집이어도 된다(그 친구의 거리는 `0`).
...
int main() {
    ...
    int kcnt;
    cin >> kcnt;
    vector<int> people(kcnt);
    for (int i = 0; i < kcnt; i++) cin >> people[i];
    long long best = INF;
    int bestX = -1;
    for (int x = 1; x <= n; x++) {
        long long total = 0;
        ...
        for (int i = 0; i < kcnt; i++) {
            if (dist[people[i]][x] == INF) { reachable = false; break; }
            total += dist[people[i]][x];
        }
...
- 어떤 친구가 `x`에 도달 불가면 그 `x`는 후보에서 탈락한다. 모든 `x`가 탈락하면 `bestX`가 `-1`로 남는다.
- 동률 처리는 "오름차순 순회 + 엄격 미만 갱신"이 정석. 복잡도 `O(N^3 + N·K)`.

(2) 코드 단계별

- `dist` 초기화(대각선 0, 무방향 양쪽, 중복 도로는 짧은 것) 후 `k → i → j` 삼중 루프.
- 친구 수 `K`와 집 목록을 읽는다(변수 이름을 `k`로 쓰면 삼중 루프의 `k`와 헷갈리니 `kcnt`로 구분했다).
- `x = 1..N`마다 친구 거리를 더하되, 한 명이라도 `INF`면 `reachable = false`로 그 `x`를 건너뛴다. 유효한 합이 `best`보다 **작을 때만** 갱신.
- 예제 1 검산: 합은 `x=1`: 0+4+3 = 7, `x=2`: 2+2+5 = 9, `x=3`: 4+0+4 = 8, `x=4`: 5+2+2 = 9, `x=5`: 3+4+0 = 7. 7이 두 번(1, 5)이지만 작은 번호 1.

(3) 스스로 다시 짤 때 생각 순서

- "Floyd로 전 쌍 거리" → "후보마다 친구 거리 합(INF 있으면 탈락)" → "엄격 미만 갱신으로 최소·동률 처리" → "없으면 -1". 경계값: 친구가 한 명이면 그 집이 답(합 0, 예제 3), 친구들이 서로 다른 덩어리에 살면 `-1`(예제 2), 같은 집에 두 명이 살면 그 거리가 두 번 더해진다.
- C++ 함정: `INF`를 20번 더하면 `2e19`로 `long long`을 넘겨 음수가 된다. 그 음수는 어떤 유효한 합보다도 작아서 "도달 못하는 동네"가 최적으로 뽑히는 최악의 오답이 된다 — `INF`는 더하지 말고 걸러 내는 것이 안전하다.
```
...
- 이미 놓인 도로 `a b`마다 대표가 다르면 합치고 `comps--`.
- 후보 도로를 정렬해 순서대로 보며 대표가 다르면 합치고 `total += w`, `comps--`. `comps == 1`이면 조기 종료.
...
- C++ 함정: `find`를 재귀로 쓰면 경로 압축 전의 트리가 길 때 스택이 깊어진다. 여기처럼 `while`로 쓰면 깊이와 무관하게 안전하다. 또 `p`를 `resize`만 하고 `p[i] = i` 초기화를 빠뜨리면 전부 0이 되어 모든 정점이 같은 덩어리로 보인다.
```
...
- **셀프체크**: 연결 그래프가 아니면 시작점 하나에서 Prim을 돌려도 일부만 편입된다. `1..N`을 훑다 **미방문 발전소를 만날 때마다** Prim을 새로 시작하면 그룹마다 MST가 하나씩 만들어진다(연결 요소 세기와 같은 구조). 힙에 넣는 값은 누적 거리가 아닌 **케이블 길이 자체**인가? 여기서도 힙은 `greater<>`를 붙인 최소 힙이어야 한다. 시작 정점의 `(0, s)`는 비용에 0을 더할 뿐이다.
...
int main() {
    ...
    for (int s = 1; s <= n; s++) {
        ...
        while (!pq.empty()) {
            ...
            for (size_t i = 0; i < graph[u].size(); i++) {
                int v = graph[u][i].first;
                long long wv = graph[u][i].second;
                if (!visited[v]) pq.push(make_pair(wv, v));
            }
        }
    }
    cout << trees << ' ' << total << '\n';
...
- `s = 1..N` 중 미방문이면 `trees++`, 힙에 `(0, s)`로 Prim 시작.
- 꺼낸 정점이 방문됐으면 스킵(지연 삭제), 아니면 편입해 `total += w`, 미방문 이웃 간선 `(wv, v)`를 push.
- 한 그룹의 while이 끝나면 힙은 비어 있으므로 같은 `pq` 객체를 다음 그룹에서 그대로 재사용해도 안전하다.
...
- C++ 함정: Prim의 힙에 넣는 값은 "시작점부터의 누적 거리"가 아니라 **간선 하나의 길이**다. Dijkstra 코드를 복사해 오면 `d + w`를 넣게 되는데, 그러면 MST가 아니라 최단 경로 트리가 만들어져 합이 달라진다. 그리고 여기서도 `greater<>`가 빠지면 최대 힙이 되어 최대 신장 트리를 구하게 된다.
```
...
int main() {
    ...
    while (!q.empty()) {
        ...
        for (size_t i = 0; i < graph[u].size(); i++) {
            int v = graph[u][i];
            if (finish[u] > finish[v]) finish[v] = finish[u];
            indeg[v]--;
            if (indeg[v] == 0) {
                finish[v] += dur[v];
                q.push(v);
...
- 독립 작업은 동시에 진행되므로 `v`는 "선행 작업 중 가장 늦게 끝나는 것"이 끝나자마자 시작한다. 즉 `finish[v] = max(모든 u→v의 finish[u]) + dur[v]` — DAG 위의 최장 경로 DP다.
- 이 점화식은 선행 작업의 `finish`가 먼저 확정돼야 하므로 위상 정렬 순서로 계산한다. Kahn 알고리즘이 그 순서를 만들어 주고, 처리 개수로 사이클도 함께 판정한다.
- 복잡도 `O(N + M)`.
...
- C++ 함정: `if (--indeg[v] == 0)`처럼 감소와 검사를 한 줄에 합칠 수 있지만, `if (indeg[v]-- == 0)`으로 잘못 쓰면 감소 **전** 값과 비교해 조건이 절대 참이 되지 않는다(진입차수가 0인 정점은 이미 큐에 들어갔으므로). 전위/후위 연산자의 차이가 조용한 오답으로 이어지는 자리다.
```
...
int main() {
    ...
    cin >> n >> m >> x;
    vector<vector<pair<int, long long>>> graph(n + 1);
    vector<vector<pair<int, long long>>> rgraph(n + 1);
    for (int i = 0; i < m; i++) {
        int u, v;
        long long w;
        cin >> u >> v >> w;
        graph[u].push_back(make_pair(v, w));
        rgraph[v].push_back(make_pair(u, w));
    }
    vector<long long> go = dijkstra(n, rgraph, x);
    vector<long long> ret = dijkstra(n, graph, x);
    long long ans = 0;
    for (int i = 1; i <= n; i++) {
        if (go[i] == INF || ret[i] == INF) {
            cout << -1 << '\n';
            return 0;
        }
        if (go[i] + ret[i] > ans) ans = go[i] + ret[i];
    }
    cout << ans << '\n';
    return 0;
}
...
- 뒤집기는 입력을 읽을 때 `rgraph[v].push_back({u, w})`로 저장하는 것뿐이다. 일방통행이므로 두 그래프는 다르다.
- 한 마을이라도 어느 방향이 `INF`면 `-1`. 복잡도 `O(2 · M log N)`.

(2) 코드 단계별

- 간선 `u→v`를 `graph[u]`에, 뒤집어 `rgraph[v]`에 넣는다. 한 번의 입력 루프에서 두 인접리스트를 동시에 채운다.
- `go = dijkstra(rgraph, X)`(집→X), `ret = dijkstra(graph, X)`(X→집).
...
- C++ 함정: 두 번째 결과 배열의 이름을 `back`으로 짓지 말자 — `using namespace std;` 아래에서는 표준 이름과 충돌하기 쉬운 자리다(`count`, `distance`, `size`, `data`, `end` 등도 마찬가지). `ret`처럼 짧고 겹치지 않는 이름을 쓴다.
```
...
- **셀프체크**: 완료 시각 계산은 9번 문제와 같다(Kahn + `max`). 여기에 "누가 `max`를 만들었는가"를 `crit[v]`에 기록하되, 동률이면 **번호가 작은 선행 공정**을 남겨야 한다 — 간선 입력 순서에 따라 결과가 달라지지 않도록 비교식을 정확히 쓰라. 마지막 공정에서 `crit`를 거꾸로 따라가 `reverse`하면 경로다. 선후 관계가 없으면 경로는 공정 하나.
...
int main() {
    ...
    while (!q.empty()) {
        ...
        finish[u] = startTime[u] + dur[u];
        for (size_t i = 0; i < graph[u].size(); i++) {
            int v = graph[u][i];
            if (finish[u] > startTime[v] || (finish[u] == startTime[v] && u < crit[v])) {
                startTime[v] = finish[u];
                crit[v] = u;
            }
            indeg[v]--;
            if (indeg[v] == 0) q.push(v);
    ...
    int last = 1;
    for (int v = 2; v <= n; v++) {
        if (finish[v] > finish[last]) last = v;
    }
    vector<int> path;
    for (int cur = last; cur != 0; cur = crit[cur]) path.push_back(cur);
    reverse(path.begin(), path.end());
    cout << finish[last] << '\n';
    for (size_t i = 0; i < path.size(); i++) {
        if (i) cout << ' ';
...
- 동률 규칙이 있으므로 갱신 조건을 정확히 써야 한다: `finish[u] > startTime[v]`이면 무조건 교체, 같으면 `u < crit[v]`일 때만 교체. 이렇게 하면 간선이 어떤 순서로 들어와도 같은 답이 나온다.
- 마지막 공정도 "완료 시각 최대, 동률이면 작은 번호"로 고르기 위해 `1..N`을 오름차순으로 돌며 엄격 초과일 때만 갱신한다. 복잡도 `O(N + M)`.

(2) 코드 단계별

- `startTime`(선행 완료 시각의 최댓값), `crit`(그 선행 공정, 없으면 0), `finish`를 준비. 진입차수 0인 공정을 큐에 넣는다.
- 큐에서 `u`를 꺼내 `finish[u] = startTime[u] + dur[u]`로 확정. 이웃 `v`마다 위 비교식으로 `startTime[v]`, `crit[v]` 갱신 후 진입차수 감소, 0이면 큐에 push.
- 처리 수가 `N` 미만이면 `-1`. 아니면 `last`를 고르고 `crit`를 따라 `0`이 나올 때까지 모아 `reverse`.
...
- "Kahn + 최장 경로" → "갱신 시 crit 기록(동률은 작은 번호)" → "last 선택(동률은 작은 번호)" → "crit 역추적·reverse". 경계값: 관계가 없으면 경로가 공정 하나(예제 3), 사이클(예제 2), 동률 선행 공정(예제 4). `crit`의 "없음"을 0으로 두려면 공정 번호가 1부터라는 점을 이용한다.
- C++ 함정: 변수 이름을 `start`나 `end`로 두면 `using namespace std;` 아래의 `std::begin`/`std::end`와 이름이 겹쳐 혼란스럽다(특히 `end`는 범위 기반 코드에서 오류를 낸다). 위 코드처럼 `startTime`, `last`로 피한다.
```
...
- 질의마다 `INF`면 `-1`; 아니면 거리와 `a`를 먼저 찍고 `cur != b`인 동안 `cur = nxt[cur][b]`로 전진하며 이어 붙인다. `a == b`면 루프가 돌지 않아 `a` 하나만 남는다.
...
- C++ 함정: 복원 루프에 종료 보장이 없다는 점을 기억하자. `nxt`가 어딘가 `0`으로 남아 있으면 `nxt[0][b]`를 읽어 범위 밖 접근이 되거나 무한 루프가 된다. `dist[a][b] != INF`를 먼저 확인했기 때문에 안전한 것이지, 확인 없이 바로 복원에 들어가면 프로그램이 멈추지 않는다.
```
