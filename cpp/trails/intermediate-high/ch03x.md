...
- **반복 훈련 개념**:
  - Kahn 뼈대: `u→v`면 `indeg[v]++` → `queue<int> q;`에 `indeg[v] == 0`인 정점을 전부 넣고 → 꺼내며 이웃의 `indeg[v]--`, 0이 되는 순간 `q.push(v)`.
  - 사전순 최소: 큐를 최소 힙으로 — `priority_queue<int, vector<int>, greater<>>`. **기본 `priority_queue<int>`는 최대 힙**이므로 `greater<>`를 빠뜨리면 정반대로 동작한다. 반대로 최대 힙이 필요하면 기본형을 그대로 쓴다(파이썬처럼 `-v`를 넣을 필요가 없다).
  - 사이클 판정: 꺼낸 정점 수가 N 미만이면 사이클 — `if ((int)order.size() < n)`. 못 꺼낸 정점이 곧 "사이클에 속하거나 사이클에 의존하는" 정점이다.
  - 위상 순서 Push DP: `u`를 꺼낸 순간 `dp[u]`는 확정 → `dp[v] = op(dp[v], dp[u] + w)`. `op`가 `max`면 최장·완료 시각, `min`이면 최단, 덧셈이면 경로 수.
  - 역그래프: 간선을 뒤집어 `rgraph[v].push_back(u)` — "끝에서부터" 계산하거나 순서를 뒤에서부터 채울 때 쓴다.
...
| 2 | 이름으로 주어진 모듈 빌드 순서 | Medium | 최소 힙 Kahn 사전순 최소(문자열 키) | 반복 훈련 |
...
int main() {
    ...
    vector<int> sem(n + 1, 1);          // 선수 없는 과목은 1학기
    queue<int> q;
    for (int v = 1; v <= n; v++) if (indeg[v] == 0) q.push(v);
    while (!q.empty()) {
        int u = q.front(); q.pop();     // 이 순간 sem[u]는 확정
        for (int v : graph[u]) {
            if (sem[u] + 1 > sem[v]) sem[v] = sem[u] + 1;
            if (--indeg[v] == 0) q.push(v);
        }
    }
    for (int v = 1; v <= n; v++) cout << sem[v] << (v == n ? '\n' : ' ');
    return 0;
}
@@TESTS
--IN
...
- `vector<vector<int>> graph(n + 1)`로 인접 리스트를, `vector<int> indeg(n + 1)`로 진입차수를 만든다(`a b`면 `b`의 진입차수 증가). 1-based라 크기를 `n + 1`로 잡는다.
- `sem`을 전부 1로 초기화하고 진입차수 0인 과목을 `queue`에 넣는다.
- `q.front()`로 값을 읽고 `q.pop()`으로 뺀다 — C++ `queue::pop()`은 값을 돌려주지 않으므로 두 줄이 한 쌍이다.
- 이웃 `v`에 `sem[v] = max(sem[v], sem[u] + 1)`을 적용하고 `--indeg[v] == 0`이면 큐에 넣는다.
- 1..N 순서로 공백 구분 출력하되 마지막만 줄바꿈.

(3) 스스로 다시 짤 때 생각 순서

- "선수 과목 + 최소 학기" → 레벨(층)을 세는 위상 정렬 = DAG 최장 경로(간선 수) DP.
- 합이 아니라 최댓값임을 먼저 확인(병렬 이수). base는 진입차수 0 = 1학기.
- C++ 함정: `graph`를 `vector<vector<int>> graph(n)`처럼 잡아 놓고 1..n을 쓰면 `graph[n]`이 범위 밖이다. `vector`의 `[]`는 검사를 하지 않아 조용히 메모리를 밟으므로 1-based면 크기를 `n + 1`로.
```
...
- **셀프체크**: 정점이 번호가 아니라 **문자열**이다 — 인접 리스트와 진입차수를 `map<string, ...>`으로 잡거나 이름→번호 매핑을 만들어라. `std::string`은 `<`가 사전순 비교라 `priority_queue<string, vector<string>, greater<string>>`에 그대로 넣으면 된다. 사전순 최소는 "지금 넣을 수 있는 것 중 가장 작은 이름"을 매번 꺼내는 heap-Kahn이며, 꺼낸 개수가 N 미만이면 `CYCLE`이다.
...
int main() {
    ...
    // 사전순 최소 → 큐를 최소 힙으로(string의 < 가 곧 사전순)
    priority_queue<string, vector<string>, greater<string>> h;
    for (const string &name : names) if (indeg[name] == 0) h.push(name);

    vector<string> order;
    while (!h.empty()) {
        string u = h.top(); h.pop();
        order.push_back(u);
...
- 정점 식별자가 문자열이라는 점만 다르다. `std::string`의 `operator<`는 사전순 비교라, `greater<string>`을 비교자로 준 `priority_queue`에 이름을 그대로 넣으면 사전순이 자동으로 지켜진다.
- 사이클이면 진입차수 0 후보가 도중에 마르므로 꺼낸 개수가 N 미만 → `CYCLE`.

(2) 코드 단계별

- 이름 목록을 읽어 `graph`(이름→후행 이름 목록)와 `indeg`(이름→진입차수)를 `map`으로 만든다. 미리 모든 이름을 키로 넣어 두면 나중에 없는 키를 읽는 사고가 없다.
- `A B`마다 `graph[a].push_back(b)`, `indeg[b]++`.
- 진입차수 0인 이름을 최소 힙에 넣고, `h.top()`으로 읽고 `h.pop()`으로 뺀다. 후행들의 진입차수를 깎아 0이 되면 push.
- `order.size() == n`이면 공백으로 이어 출력, 아니면 `CYCLE`.

(3) 스스로 다시 짤 때 생각 순서

- "빌드 순서 + 사전순" → heap-Kahn. 번호 대신 문자열이므로 자료구조를 `map<string, ...>`으로 바꾸는 것만 결정하면 된다.
- C++ 함정: `map`의 `operator[]`는 **키가 없으면 기본값으로 만들어 넣는다**. `indeg[b]++`는 그래서 잘 동작하지만, 존재 여부만 확인하고 싶을 때 `[]`를 쓰면 원소가 슬그머니 늘어난다. 확인만 할 때는 `count`나 `find`를 쓴다.
- 방향 실수 주의: `A B`는 A→B, 진입차수는 B에 센다. M=0이면 모든 이름이 처음부터 힙에 들어가 정렬 결과와 같아진다.
```
...
int main() {
    ...
    cout << cnt << '\n';
    vector<int> blocked;
    for (int v = 1; v <= n; v++) if (!done[v]) blocked.push_back(v);
    if (blocked.empty()) {
        cout << "NONE" << '\n';
    } else {
        for (size_t i = 0; i < blocked.size(); i++)
            cout << blocked[i] << (i + 1 == blocked.size() ? '\n' : ' ');
    }
    return 0;
}
@@TESTS
...
- 인접 리스트와 진입차수를 만든다.
- 진입차수 0인 정점으로 큐를 시작. 꺼낼 때마다 `done[u] = 1`, 개수 증가, 이웃 진입차수 감소.
- 끝난 뒤 `done`이 0인 번호를 오름차순으로 모아 둘째 줄에 출력(없으면 `NONE`).

(3) 스스로 다시 짤 때 생각 순서

- "사이클 + 그에 의존하는 것까지" → Kahn에서 못 꺼낸 정점이 정확히 그 집합임을 떠올린다.
- 정답이 유일하도록 오름차순으로 출력하는 요구를 지킨다(1..n 순회면 자동).
- C++ 함정: `blocked.size()`는 부호 없는 `size_t`다. 마지막 원소 판정을 `i == blocked.size() - 1`로 쓰면 벡터가 비었을 때 `0 - 1`이 거대한 양수가 된다. 여기서는 `i + 1 == blocked.size()`로 뺄셈을 피했고, 빈 경우도 먼저 분기해 두었다.
...
int main() {
    ...
    while (!q.empty()) {
        if (q.size() > 1) unique_order = false;   // 동시에 놓을 수 있는 정점이 둘 이상
        int u = q.front(); q.pop();
        order.push_back(u);
        for (int v : graph[u])
            if (--indeg[v] == 0) q.push(v);
...
- 위상 순서가 유일하려면 매 단계에 "지금 놓을 수 있는 정점"이 정확히 하나여야 한다. 둘 이상이면 그 둘의 상대 순서를 바꾼 다른 위상 순서가 존재한다. 그래서 Kahn에서 정점을 꺼내는 순간 `q.size() > 1`이면 유일하지 않다.
...
- 진입차수 0인 정점을 큐에 넣고, 꺼내기 직전에 `q.size() > 1`이면 `unique_order = false`.
- 꺼낸 정점을 `order`에 넣고 이웃 진입차수를 깎는다.
- 결과 판정 순서: 개수 부족 → `IMPOSSIBLE`, `unique_order`가 false → `?`, 아니면 `order` 출력.

(3) 스스로 다시 짤 때 생각 순서

- "순서가 유일한가" → 큐 크기 관찰이라는 한 줄 추가로 Kahn이 답한다.
- 세 결과의 우선순위(사이클 > 불확정 > 유일)를 먼저 정해두고 마지막에 판정한다.
- C++ 함정: 플래그 변수를 `unique`로 지으면 `using namespace std;` 아래에서 `std::unique` 알고리즘과 이름이 겹쳐 읽는 사람을 혼란스럽게 한다(`sort`, `count`, `distance`, `size`, `data`도 같은 부류다). 여기서는 `unique_order`로 피했다.
...
- **셀프체크**: 음수 가중치가 있으면 다익스트라는 못 쓰지만 DAG에서는 위상 순서 DP가 그대로 통한다(각 정점을 한 번만 확정하므로 음수도 문제없음). `dist`를 `INT_MAX`로 초기화하고 **`dist[u] == INT_MAX`인 정점에서는 절대 완화하지 마라** — 파이썬의 `inf + w`는 여전히 `inf`지만 C++의 `INT_MAX + w`는 오버플로해 음수로 돌아 조용히 틀린다. 출발점이 아닌 진입차수 0 정점(예제의 5)도 Kahn 큐에는 넣어야 위상 순서가 완성된다.
...
int main() {
    ...
    int n, m, s;
    cin >> n >> m >> s;
    vector<vector<pair<int, int>>> graph(n + 1);   // {도착, 가중치}
    vector<int> indeg(n + 1, 0);
    for (int i = 0; i < m; i++) {
        int u, v, w;
        cin >> u >> v >> w;
    ...
    while (!q.empty()) {
        ...
        for (auto &[v, w] : graph[u]) {
            // INF에서 완화하면 INT_MAX + w가 오버플로한다 — 반드시 먼저 거른다
            if (dist[u] != INF && dist[u] + w < dist[v]) dist[v] = dist[u] + w;
            if (--indeg[v] == 0) q.push(v);
        }
    }
    for (int v = 1; v <= n; v++) {
        if (dist[v] == INF) cout << 'X';
        else cout << dist[v];
        cout << (v == n ? '\n' : ' ');
    }
    return 0;
...
- 도달 불가와 비용 0은 다르다 — `INT_MAX`로 초기화해 구분하고 출력 시 `X`로 바꾼다.

(2) 코드 단계별

- `N M S`를 읽고 `graph[u] = {{v, w}, ...}`와 진입차수를 만든다.
- `dist[S] = 0`, 나머지 `INT_MAX`. 진입차수 0인 모든 정점(출발점이 아니어도)을 큐에 넣는다.
- Kahn으로 `u`를 꺼내며 `dist[u] != INF`일 때만 이웃을 완화하고, 진입차수를 깎아 0이 되면 큐에.
- 1..N 순서로 출력하되 `INT_MAX`는 `X`.

(3) 스스로 다시 짤 때 생각 순서

- "DAG + 음수 가중치 + 출발점 고정" → 다익스트라 대신 위상 순서 DP(min). 최장 경로 문제에서 max를 min으로 바꾼 대칭형.
- C++ 함정: 파이썬의 `float('inf')`는 아무리 더해도 `inf`라 습관적으로 그냥 더해도 됐지만, C++의 `INT_MAX`는 유한한 값이라 1만 더해도 부호 있는 오버플로 — **정의되지 않은 동작**이다. 그래서 `1e9`처럼 여유 있는 INF를 쓰거나(더해도 `int`를 안 넘음) 여기처럼 완화 전에 걸러야 한다.
...
- **셀프체크**: K를 지나는 경로는 "S→K 경로" 하나와 "K→T 경로" 하나의 이어붙임이고, DAG라 K를 두 번 지날 수 없으므로 **곱**으로 센다: `paths(S→K) × paths(K→T)`. 후자는 역그래프에서 T를 출발점으로 삼은 경로 수 DP로 얻는다. 두 번의 Kahn은 각각 **별도의 진입차수 배열**을 써야 한다 — C++에서 `vector<int> deg = indeg;`는 복사라 안전하지만 `vector<int> &deg = indeg;`로 받으면 원본이 깎여 두 번째 Kahn의 큐가 비어 버린다. 마지막 곱셈은 `long long`으로 받았는가(1e9 × 1e9는 `int`를 훨씬 넘는다)? K가 S에서 도달 불가면 0, K=T면 `paths(K→T)=1`.
...
int main() {
    ...
    vector<int> indeg(n + 1, 0), rindeg(n + 1, 0);
    for (int i = 0; i < m; i++) {
        int u, v;
        cin >> u >> v;
        graph[u].push_back(v);
        indeg[v]++;
        rgraph[v].push_back(u);            // 역그래프
        rindeg[u]++;
    }
    // dp[v] = src에서 v로 가는 경로 수 (넘긴 graph 방향 기준)
    auto count_from = [&](const vector<vector<int>> &g, const vector<int> &deg0, int src) {
        vector<long long> dp(n + 1, 0);
        dp[src] = 1;
        vector<int> deg = deg0;            // 복사 — 원본 진입차수는 보존된다
        queue<int> q;
    ...
    vector<long long> f = count_from(graph, indeg, s);     // f[v] = S→v 경로 수
    vector<long long> g = count_from(rgraph, rindeg, t);   // g[v] = v→T 경로 수
    cout << f[k] * g[k] % MOD << '\n';
    return 0;
}
@@TESTS
...
- `count_from(g, deg0, src)` 람다: 진입차수를 `vector<int> deg = deg0;`로 **값 복사**해 쓴다. 큰 컨테이너를 인자로 받을 때는 `const vector<...> &`로 받아 복사를 막고, 안에서 고쳐야 하는 것만 따로 복사하는 것이 C++ 관용구다.
- `dp[src] = 1`에서 시작해 Kahn Push로 `dp[v] += dp[u]`(매번 `% MOD`).
- 같은 람다를 `rgraph`, `rindeg`, `T`로 다시 호출해 `g`를 얻고 `f[k] * g[k] % MOD` 출력.

(3) 스스로 다시 짤 때 생각 순서

- "반드시 지나는" → 경유 정점에서 경로를 둘로 쪼개 곱한다(DAG라 중복 통과 없음).
- 뒤쪽 절반은 역그래프 DP라는 도구를 떠올린다. 경로 수 DP 함수를 하나 만들어 두 번 재사용.
- C++ 함정: `dp`를 `vector<int>`로 두면 `f[k] * g[k]`가 최대 (1e9)² ≈ 1e18로 `int` 곱셈에서 오버플로한다. `long long`으로 두거나 `(long long)f[k] * g[k] % MOD`처럼 한쪽을 캐스팅해야 한다. 덧셈은 mod 덕에 안전해도 **곱셈은 늘 따로 확인**하라.
- 경계: K 도달 불가(0), K=T(g=1) 같은 경우도 곱셈 공식이 그대로 처리한다.
```
...
- **셀프체크**: 두 번 훑는다. (a) 정방향(위상 순서)으로 가장 빠른 시작 `ES[v] = max(ES[u] + t[u])`, `T = max(ES[v] + t[v])`. (b) 역순(위상 순서를 뒤집어)으로 가장 늦은 완료 `LF[v] = min(LF[w] - t[w])`(후행 w), 후행이 없으면 `T`. 여유 = `(LF[v] - t[v]) - ES[v]`. 여유가 0인 작업들이 임계 경로다. 정방향 결과인 위상 순서 배열을 저장해 두고 **뒤집어**(`rbegin/rend` 또는 인덱스 역주행) 쓰면 역그래프를 따로 만들 필요가 없다.
...
int main() {
    ...
    // (a) 정방향: 가장 빠른 시작 es, 그리고 위상 순서 order
    vector<int> es(n + 1, 0), order;
    order.reserve(n);
    queue<int> q;
    for (int v = 1; v <= n; v++) if (indeg[v] == 0) q.push(v);
    while (!q.empty()) {
        ...
        order.push_back(u);
        for (int v : graph[u]) {
            if (es[u] + t[u] > es[v]) es[v] = es[u] + t[u];
            if (--indeg[v] == 0) q.push(v);
        }
    }
    int total = 0;
    for (int v = 1; v <= n; v++) total = max(total, es[v] + t[v]);
    // (b) 역순: 가장 늦은 완료 lf (후행이 없으면 total)
    vector<int> lf(n + 1, total);
    for (int i = (int)order.size() - 1; i >= 0; i--) {
        int u = order[i];
        for (int v : graph[u])
            if (lf[v] - t[v] < lf[u]) lf[u] = lf[v] - t[v];
    }
    cout << total << '\n';
    for (int v = 1; v <= n; v++)
        cout << (lf[v] - t[v] - es[v]) << (v == n ? '\n' : ' ');
    return 0;
}
@@TESTS
--IN
...
- `t`, 그래프, 진입차수를 읽는다.
- Kahn으로 `es`를 채우면서 꺼낸 순서를 `order`에 기록한다. `total = max(es[v] + t[v])`.
- `lf`를 `total`로 초기화하고 `order`를 뒤에서부터 순회: `u`의 각 후행 `v`에 대해 `lf[u] = min(lf[u], lf[v] - t[v])`. 역순에서는 `v`가 `u`보다 먼저 처리되어 `lf[v]`가 확정돼 있다.
...
- C++ 함정: 역순 순회를 `for (size_t i = order.size() - 1; i >= 0; i--)`로 쓰면 `size_t`가 부호 없는 타입이라 `i >= 0`이 항상 참이고 0에서 한 번 더 감소해 거대한 값으로 감싸며 무한 루프·범위 밖 접근이 된다. `int`로 캐스팅하거나 `for (auto it = order.rbegin(); it != order.rend(); ++it)`를 쓴다.
...
- **셀프체크**: 이 문제는 **사전순 최소와 다르다**. 첫 예제의 사전순 최소는 `2 3 1`이지만, 1번을 최대한 앞으로 보내려면 3을 먼저 처리하고 바로 1을 놓는 `3 1 2`가 답이다. 정석은 **역그래프에서 최대 힙 Kahn을 돌려 얻은 순서를 뒤집는 것**: 뒤에서부터 "지금 마지막에 놓을 수 있는 것 중 가장 큰 번호"를 고르면 큰 번호가 뒤로 밀리고, 결과적으로 작은 번호가 앞당겨진다. C++의 기본 `priority_queue<int>`가 이미 최대 힙이라 음수를 넣는 파이썬식 뒤집기가 필요 없다. 사이클 판정은 역그래프에서도 "꺼낸 개수 < N"으로 동일하다.
...
int main() {
    ...
    int n, m;
    cin >> n >> m;
    vector<vector<int>> rgraph(n + 1);        // 역그래프: v -> u
    vector<int> rindeg(n + 1, 0);
    for (int i = 0; i < m; i++) {
        int u, v;
        cin >> u >> v;
    ...
    priority_queue<int> h;                    // 기본이 최대 힙
    for (int v = 1; v <= n; v++) if (rindeg[v] == 0) h.push(v);
    vector<int> order;
    order.reserve(n);
    while (!h.empty()) {
        int u = h.top(); h.pop();
        order.push_back(u);
        for (int v : rgraph[u])
            if (--rindeg[v] == 0) h.push(v);
    }
    if ((int)order.size() < n) {
        cout << -1 << '\n';
    } else {
        reverse(order.begin(), order.end());  // 뒤에서부터 채운 것을 뒤집는다
        for (int i = 0; i < n; i++) cout << order[i] << (i + 1 == n ? '\n' : ' ');
    }
    return 0;
}
...
- 간선 `u v`를 읽되 **역방향**으로 저장한다: `rgraph[v].push_back(u)`, `rindeg[u]++`.
- 역그래프 진입차수 0인 정점을 `priority_queue<int>`(기본 최대 힙)에 넣는다.
- 꺼낸 정점을 `order`에 추가하고, 역그래프 이웃의 진입차수를 깎아 0이 되면 push.
- 꺼낸 개수가 N 미만이면 `-1`, 아니면 `reverse(order.begin(), order.end())` 후 출력.
...
- C++ 함정: 최소 힙이 필요할 때만 `greater<>`를 붙인다는 점을 기억해 두면 이 문제에서는 오히려 아무것도 안 붙이는 게 정답이다. 파이썬 `heapq` 코드를 옮길 때 `-v`를 그대로 남기고 `priority_queue<int>`를 쓰면 부호가 두 번 뒤집혀 최소 힙이 되어 버린다.
